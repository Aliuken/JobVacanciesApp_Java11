...
function treatFilterAndPaginationCombosAndUrlParameters(languageSessionAttribute, languageModelAttribute, defaultLanguage, filterNameModelAttribute, filterValueModelAttribute, sortingFieldModelAttribute, sortingDirectionModelAttribute, pageSizeModelAttribute, pageNumberModelAttribute) {
  ...
  //Set the pageSizeCombo to the value of the pageSize URL param (if the pageSizeCombo exists)
  let pageSizeCombo = getComboElementIfExists("pageSizeCombo");
  if(pageSizeCombo != null) {
    //When the pageSizeCombo element change, set the value of the pageSize URL param to the new selected value
...
function resetPageNumberParameterInUrl() {
  let urlSearchParams = new URLSearchParams(window.location.search);
  let filterValueParam = getParameterFromUrlSearchParams(urlSearchParams, "filterValue");
  let pageNumberParam = getParameterFromUrlSearchParams(urlSearchParams, "pageNumber");

  let filterValueElem = getElementIfExists("filterValue");
  let filterValueElemCurrentVal = filterValueElem.val();

  let resetDone = (pageNumberParam !== "0" || filterValueParam !== filterValueElemCurrentVal);
  if(resetDone) {
    addOrReplaceParameterInUrlSearchParams(urlSearchParams, "filterValue", filterValueElemCurrentVal);
    addOrReplaceParameterInUrlSearchParams(urlSearchParams, "pageNumber", "0");
    window.location = window.location.protocol + "//" + window.location.host + window.location.pathname + "?" + urlSearchParams.toString();
  }
  return resetDone;
}
