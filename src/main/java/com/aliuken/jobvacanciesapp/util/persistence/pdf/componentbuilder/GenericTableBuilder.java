...
public class GenericTableBuilder implements PdfPTableEvent {
	...
	public GenericTableBuilder(final String columnName, final String columnValue, final float columnWidth) throws DocumentException {
		this(new String[]{columnName}, new String[]{columnValue}, new float[]{columnWidth}, null);
	}

	//Single-text content constructor
	public GenericTableBuilder(final Font cellFont, final String contentArrayValue) throws DocumentException {
		this(null, null, NO_HEADER_WIDTH, new GenericTableContentDTO(Element.ALIGN_CENTER, cellFont, new String[][]{{contentArrayValue}}, false, false));
	}

	public GenericTableBuilder(final String[] columnNames, final String[] columnValues, final float[] columnWidths, final GenericTableContentDTO genericTableContentDTO) throws DocumentException {
		if(columnNames != null) {
		...
		if(this.isHeaderWithContent) {
			this.cellHorizontalAlignment = genericTableContentDTO.getCellHorizontalAlignment();
			this.cellFont = genericTableContentDTO.getCellFont();
			this.cellFont.setColor(new BaseColor(2, 2, 2));
			this.contentArray = genericTableContentDTO.getContentArray();

			if(genericTableContentDTO.isAlternateRowColor()) {
			...
			this.drawBorders = genericTableContentDTO.isDrawBorders();
		} else {
	...
	private void addHeaderRow(final PdfPTable pdfPTable) {
		if(LogicalUtils.isNotNullNorEmpty(columnNames)) {
			...
			for(int i = 0; i < columnNames.length; i++) {
				...
				final GenericTableHeaderCellBuilder genericTableHeaderCellBuilder = new GenericTableHeaderCellBuilder(columnName, columnValue, isHeaderWithContent, defaultCell);
