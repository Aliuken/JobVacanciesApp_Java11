...
@Controller
@Slf4j
public class JobCategoryJobVacancyController {
	...
	public String getJobVacancies(Model model, Pageable pageable, @PathVariable("jobCategoryId") long jobCategoryId,
		...
		try {
			if(tableSearchDTO == null || tableSearchDTO.hasEmptyAttribute()) {
				...
				return MvcUtils.getNextViewWithTable("jobCategory/jobCategoryJobVacancies.html", model, operation, tableSearchDTO, false);
			}
			...
			if(bindingResult.hasErrors()) {
				...
				return MvcUtils.getNextViewWithTable("jobCategory/jobCategoryJobVacancies.html", model, operation, tableSearchDTO, true);
			}
	...
	public String delete(RedirectAttributes redirectAttributes, @PathVariable("jobCategoryId") long jobCategoryId, @PathVariable("jobVacancyId") long jobVacancyId,
		...
		return MvcUtils.getNextRedirectWithTable("/job-categories/job-vacancies/" + jobCategoryId, languageCode, tableFieldCode, tableFieldValue, tableOrderCode, pageSize, pageNumber);
	}
