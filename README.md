...
The database of the application comes with 8 predefined users with the following credentials and roles:

```txt
| Email                 | Password | Role          |
|-----------------------|----------|---------------|
| anonymous@aliuken.com |          |               |
...
NOTES:
* The users **anonymous@aliuken.com** is used for the operations when the user is still not logged in. You cannot log in as the anonymous user as it doesn't have password and role.
