...
The code was created using the **SOLID principles** and the **SOA architecture**.
...
* [JobVacanciesApp_Java17](https://github.com/Aliuken/JobVacanciesApp_Java17) and [JobVacanciesApp_AppData_Java17](https://github.com/Aliuken/JobVacanciesApp_AppData_Java17), which use **Java 17** and **Spring Boot 3.2**.
...
* **Singleton**: It is used when we want a single instance of a class and it is not possible to use enums, @Autowired or utility classes (with only static methods). This pattern is used in the packages:
    ...
    * [com.aliuken.jobvacanciesapp.util.javase.stream](https://github.com/Aliuken/JobVacanciesApp_Java11/tree/main/src/main/java/com/aliuken/jobvacanciesapp/util/javase/stream): To get StreamUtils instances (of type SequentialStreamUtils or ParallelStreamUtils) statically to execute its instance methods.
...
* **OpenJDK 11**: As the Java SE implementation.
...
* **Internationalization** (**i18n**): Configured in [LocaleConfig](https://github.com/Aliuken/JobVacanciesApp_Java11/blob/main/src/main/java/com/aliuken/jobvacanciesapp/config/LocaleConfig.java) (as in <https://www.baeldung.com/spring-boot-internationalization>) and built for:
...
* **Spring MessageSource class**: To deal with internationalization (i18n). It is configured in [LocaleConfig](https://github.com/Aliuken/JobVacanciesApp_Java11/blob/main/src/main/java/com/aliuken/jobvacanciesapp/config/LocaleConfig.java).
...
* **Thymeleaf**: As the HTML5 template engine (configured in [WebTemplateConfig](https://github.com/Aliuken/JobVacanciesApp_Java11/blob/main/src/main/java/com/aliuken/jobvacanciesapp/config/WebTemplateConfig.java)), using:
    * The decorator **tempate.html** (which uses **thymeleaf-layout-dialect** to create the layout template).
...
* **Bootstrap 5.3.2** and **Material Design for Bootstrap 7.1.0**: For the look-and-feel.
* **Font Awesome Free For The Web 6.5.1**: For the application icons.
* **jQuery 3.7.1**: To make an easier use of **JavaScript**.
* **jQuery UI 1.13.2** and **jQuery Timepicker Addon 1.6.3**: For the calendar UI-input element.
* **TinyMCE Community 6.8.2**: For the rich text editor.
* The following **ad-hoc static files** (located in [src/main/resources/static/jobvacanciesapp-utils](https://github.com/Aliuken/JobVacanciesApp_Java11/tree/main/src/main/resources/static/jobvacanciesapp-utils)):
    ...
    * **rich-text-editor.css** and **rich-text-editor.js**: For the rich text editor. The TinyMCE ["Full Featured: Non-Premium Plugins"](https://www.tiny.cloud/docs/demo/full-featured/#fullfeaturednon-premiumplugins) CSS and JS code was copied in those files.
...
> * **tablePagination**: Which is used to show the pagination buttons in pages that have a table. Those buttons are shown below the table.
>
> The DTO **TableSearchDTO** is used to store the URL parameters regarding filter, sorting and pagination in pages that have a table. Those parameters are:
> * **languageParam**: Which is the selected language (one of the values defined in **Language**).
> * **tableFieldCode**: Which is the filtered field name (one of the values defined in **TableField**).
> * **tableFieldValue**: Which is the filtered field value.
> * **tableOrderCode**: Which is a value that determines the field to be sorted and the sort direction (one of the values defined in **TableOrder**).
> * **pageSize**: Which the size of each page (one of the values defined in **TablePageSize**).
...
* **FileUtils**, **FileNameUtils** and **FileType**: To manage the persistence in files (of logos and curriculums).
...
> * **As an image** in: [documentation/Entity-Relationship-Diagram.drawio.png](https://github.com/Aliuken/JobVacanciesApp_Java11/blob/main/documentation/Entity-Relationship-Diagram.drawio.png).
...
> * **ControllerAspectRestUtils**: Used in ControllerAspect to get the RequestMapping, GetMapping or PostMapping from the JoinPoint.
> * **ControllerAspectLoggingUtils**: Used in ControllerAspect to log multiple stats.
> * **RepositoryAspectLoggingUtils**: Used in RepositoryAspect to log multiple stats.
...
* **Docker Compose for the Elastic Stack** (in [docker-compose-elastic-stack/docker-compose.yaml](https://github.com/Aliuken/JobVacanciesApp_Java11/blob/main/docker-compose-elastic-stack/docker-compose.yaml)). It uses:
    * The **Elastic Stack 8.12.0** Docker images (url: <https://www.docker.elastic.co>): For analyzing the app log files, which pass through the stack in the next order: "Filebeat &rArr; Logstash &rArr; Elasticsearch &rArr; Kibana".
...
* **jobvacanciesapp.defaultLanguageCode**: Indicates the default application language. Initial value: "en". Possible values (defined in the enum **Language**):
...
* **jobvacanciesapp.defaultInitialTablePageSizeValue**: Indicates the default initialTablePageSize. Initial value: "5". Possible values (defined in the enum **TablePageSize**):
...
* **jobvacanciesapp.defaultColorModeCode**: Indicates the default colorMode. Initial value: "L". Possible values (defined in the enum **ColorMode**):
...
* **jobvacanciesapp.userInterfaceFrameworkCode**: Indicates the UI framework of the application. Initial value: "M". Possible values (defined in the enum **UserInterfaceFramework**):
